...
for ii = 1:L
    ...
    for jj = 1: nloop
        ...
        %***************  attn ***********************
        attn_2 = sqrt( 10.^(Gsnrdb(ii)/10) * nSample / sum(abs(txSignal).^2));  % Signal amplitude
                
        %**********  received signal except noise  *******************************************
        rxSignal = txSignal  *  attn_2;       %  Eq.(1)  in H1
        rxSignal_2 = 0 ;                      %  Eq.(1)  in H0
        ...
        for k = 1:1:N
            %*************   noise  ************************
            noise = alpha_channel(nSample,alpha,gama,beta,a) ;  % alpha  noise
            %*************  Signal + noise  ***********************
            RX(:,k) = rxSignal + noise' ;  % received signals in H1, 0% is corrupted 
            RX_2(:,k) = rxSignal_2 + noise' ;  % received signals in H0
            
            %*********************  corruption scenarios  ********************************          
            RX(1:nSample/3,k) = noise(1:nSample/3)';  %  30%  is corrupted        
%             RX(1:nSample*2/3,k) = noise(1:nSample*2/3)';  %  60%  is corrupted
        end %for k
