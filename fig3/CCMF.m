    ...
    for jj = 1: nloop
       ...
       %% ********* calculate max correntropy in H1 *************************************
        for  i = 1:1:1000   %1000 
            ...
            %************ J *****************
            J1 = sum( exp(-1/(2 * delta.^2) * ( rxSignal - RX * kxi ).^2 )) - epsilon * sum( abs (kxi) );   %Eq.(17)
...
%             kxi = kxi + 0.1 ;
       ...
       %% ********* calculate max correntropy in H0 *************************************
        for  j = 1:1:1000   %1000 
            ...
            %************ J *****************
            J1_2 = sum( exp(-1/(2 * delta.^2) * ( rxSignal - RX_2 * kxi_2 ).^2 )) - epsilon * sum( abs (kxi_2)); %Eq.(17)
