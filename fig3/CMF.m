...
clc
clear
disp(['The simulation of MaxCorrentyopy vs Correntropy  in alpha noise '])
disp([' alpha£¬CMF , Eq.(10)'])
disp([])
...
%         attn = sqrt(attn);                      %  attn of noise
       ...
       %% ********* calculate max correntropy in H1 *************************************
        for  i = 1:1:1000   %1000 
            ...
            %************  J *****************
            J1 = sum( exp(-1/(2 * delta.^2) * ( rxSignal - RX * kxi ).^2 )) - epsilon * sum( abs (kxi) ); %Eq.(10)
       ...
       %% ********* calculate max correntropy in H0 *************************************
        for  j = 1:1:1000   %1000 
            ...
            %************  J *****************
            J1_2 = sum( exp(-1/(2 * delta.^2) * ( rxSignal - RX_2 * kxi_2 ).^2 )) - epsilon * sum( abs (kxi_2));  %Eq.(10) 
       ...
       %% ********* calculate  correntropy  *********************            
        ...
        %************ J ************************               
        J1_3 = sum( exp(-1/(2 * delta.^2) * ( rxSignal - RX_3 ).^2 )) ;
        esti_J_3 = 1/( sqrt(2*pi) * delta * nSample) * J1_3;     
        
        J1_4 = sum( exp(-1/(2 * delta.^2) * ( rxSignal - RX_4  ).^2 )) ;
...
%***************  output result  ***************
figure(1);
xlabel('False alarm probability,{\it P_f}  , {\it v } = 1 ');
ylabel('Detection probability,{\it P_d }');

plot(pf_2(:,1),pd_2(:,1),'s:b',  ...
     pf_2(:,2),pd_2(:,2),'+:b', ...
     pf_2(:,3),pd_2(:,3),'*:b', 'LineWidth',2);

leg = legend('CMF,{\it GSNR }= -10dB','CMF,{\it GSNR }=  -5dB','CMF,{\it GSNR }=  0dB');
